using System;
using System.IO;
using NUnit.Framework;
using Electrolyte.Messages;

namespace Electrolyte.Test.Messages {
	[TestFixture]
	public class TransactionTest {
		[Test]
		public void TestReadTransactionFromBinaryStream() {
			byte[] bytes = {
				
			};

			using(BinaryReader reader = new BinaryReader(new MemoryStream(bytes))) {
				Transaction tx = new Transaction(reader);

				Assert.AreEqual(tx.Version, 1);

				Assert.AreEqual(tx.Inputs.Count, 1);
				Assert.AreEqual(tx.Inputs[0].Sequence, UInt32.MaxValue);

				Assert.AreEqual(tx.Outputs.Count, 2);
				Assert.AreEqual(tx.Outputs[0].Value, 5000000);
				Assert.AreEqual(tx.Outputs[1].Value, 3354000000);

				Assert.AreEqual(tx.LockTime, 0);
				Assert.AreEqual(tx.Value, 3359000000);
			}
		}
	}
}

